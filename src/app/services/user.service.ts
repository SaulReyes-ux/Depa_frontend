...
export class UserService {
  ...
  public async userVerificated(user : User) : Promise<boolean> {
    try {
      return false;
      const userVerificated = await lastValueFrom(this.httpService.get(this.USER_URL + 'verificated', user)) as boolean
  ...
  public async sendVerificationCode(username: string): Promise<void> {
    try {
      await lastValueFrom(this.httpService.post(this.USER_URL + 'send-verification-email', { username }));
    } catch (err) {
      throw err;
    }
  }

  public async verifyUser(code: number, user : User): Promise<boolean> {
    //Traer el codigo de verificacion del usuario y comparar
    //Si el codigo es correcto, actualizar el usuario como verificado
    try {
      const userVerificated = await lastValueFrom(this.httpService.post(this.USER_URL + 'verify-user', { code, user })) as boolean
      return userVerificated
    } catch (err) {
      throw err
    }
