...
export class HeaderFilterComponent implements OnInit, OnDestroy {
  ...
  getGuests() {
    let str = this.order.guests.adults + this.order.guests.children > 0 ? (this.order.guests.adults + this.order.guests.children) + this.getTranslation('guests') : ''
    str += this.order.guests.infants > 0 ? ' ,' + this.order.guests.infants + this.getTranslation('infants') : ''
    str += this.order.guests.pets > 0 ? ' ,' + this.order.guests.pets + this.getTranslation('pets') : ''
    return str
  }
